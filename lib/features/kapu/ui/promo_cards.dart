...
class _PromoCardsSwiperPageState extends State<PromoCardsSwiperPage> {
  ...
  Widget _buildStateChild(KapuState state) {
    if (_hasFetchedOnce && _merchantBalances.isNotEmpty) {
      return _buildSwiper(
        [],
      ); // we’ll use local map, so wallets param not needed
    }
                ...
                child: AnimatedContainer(
                  ...
                  child: GestureDetector(
                    onTap: () async {
                      ...
                      try {
                        // 🟢 Step 1: Create booking before navigating...mark the user
                        ...
                        // 🟢 Fetch shops in the background
                        _kapuCubit.fetchKapuShops(merchantId);

                        // 🟢 Only navigate if booking was successfully created
                        if (booking != null) {
                          ...
                          if (shouldRefresh == true && mounted) {
                            final merchantIds = merchants
                                ...
                                .toList();
                            _kapuCubit.fetchMultipleKapuWalletBalances(
                              merchantIds,
                            );
                          }
                        }
